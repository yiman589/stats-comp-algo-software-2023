...
**Your answer:** The size of the folder is 346K while the total size of the individual files inside it is 76K. This is the case because there is a hidden folder called ".git" inside the git_experiment when initializing the git environment that holds all the log and process information ready to be committed to github. This would explain why Github discourages from committing large binary files because for a large folder, the ".git" folder would be even larger to keep all the information and thus results in a possibly unhealthy repository environment.
...
https://github.com/yiman589/hiperglm.git