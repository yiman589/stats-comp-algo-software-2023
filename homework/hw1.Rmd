...
**Your answer:** The size of the folder is 5.3M while the total size of the individual files inside it is 113K. This is the case because the hidden folder “.git” inside the git_experiment keeps the record for all the time log information and commitment when we go through each loop. This would explain why Github discourages from committing large binary files because for a large folder, the memory needs would be even bigger to keep all the records and thus results in a possibly unhealthy repository environment.
